        ...
        active_peer_bindings = [
            {
                ...
                "rx_packets": int(state.get("rx_packets", 0) or 0),
                "rx_bytes": int(state.get("rx_bytes", 0) or 0),
                "tx_packets": int(state.get("tx_packets", 0) or 0),
                "tx_bytes": int(state.get("tx_bytes", 0) or 0),
                "learned_ipv4": [str(v) for v in list(state.get("learned_ipv4") or [])],
                "learned_ipv6": [str(v) for v in list(state.get("learned_ipv6") or [])],
                "throttle_prev_window_bytes": 0,
                "throttle_curr_window_bytes": 0,
                "throttle_drop_count": 0,
                "local_virtual": False,
