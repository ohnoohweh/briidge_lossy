...
final class ObstacleBridgeTunnelControl: NSObject {
    ...
    private static var providerConfigurationVersion: String {
        ...
        return "\(providerConfigurationBaseVersion)+\(providerBuildTimestampUTC)"
    }
    ...
    private class func selectCanonicalManager(
        ...
        let desiredManagers = managers.filter { hasCurrentProviderConfiguration($0) }
            .sorted { a, b in
        ...
        if let canonical = desiredManagers.first {
            let duplicates = managers.filter { $0 !== canonical }
        ...
        if !managers.isEmpty {
            let canonical = managers.sorted { canonicalScore(for: $0) > canonicalScore(for: $1) }.first!
            let duplicates = managers.filter { $0 !== canonical }
            return (
                manager: canonical,
                duplicates: duplicates,
    ...
    private class func desiredLocalizedDescription() -> String {
        let filteredScalars = ObstacleBridgeGeneratedBuildStamp.providerBuildTimestampUTC.unicodeScalars.filter {
    ...
    private class func removeDuplicateManagers(
        ...
        for manager in managers {
            ...
            case .connected, .connecting, .reasserting, .disconnecting:
                ...
                updateStatus(ok: true, event: "duplicate_cleanup_stop_requested", manager: manager)
            default:
                break
            }
            group.enter()
    ...
    private class func deriveTunnelNetworkSettings() -> DerivedTunnelNetworkSettings {
        ...
        if !overlayPeerConfigured(payload: flattenedPayload) {
            ...
            let overridden = routingOverride.map { onboarding.applying($0) } ?? onboarding
            let nonCapturing = ObstacleBridgeDerivedTunnelSettings(
            ...
            return DerivedTunnelNetworkSettings(shared: nonCapturing)
        }
    ...
    private class func syncDecision(
        ...
        case (true, true):
            let appConfigured = isSemanticallyConfiguredConfig(atPath: appConfig.path)
            let sharedConfigured = isSemanticallyConfiguredConfig(atPath: sharedConfig.path)
            if appConfigured != sharedConfigured {
                if sharedConfigured {
                    return ("shared_semantically_newer", false, true)
                }
                return ("app_semantically_newer", true, false)
            }
    ...
    private class func isSemanticallyConfiguredConfig(atPath path: String) -> Bool {
        ...
        if overlayPeerConfigured(payload: flattened) {
            return true
        }
        let ownServices = ObstacleBridgeRuntimeConfig.ownServerSpecs(from: payload, preserveInputIndices: true)
        if !ownServices.isEmpty {
            return true
        }
        let remoteServices = ObstacleBridgeRuntimeConfig.remoteServerSpecs(from: payload, preserveInputIndices: true)
