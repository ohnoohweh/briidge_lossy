...
final class ObstacleBridgeTunnelControl: NSObject {
    ...
    private static let providerBuildTimestampUTC = "2026-05-26T12:27:45Z"
    ...
    private static let defaultTunnelAddress = "192.168.105.1"
    ...
    private static let defaultTunnelAddress6 = ""
    ...
    private static let defaultMTU = 1280
    ...
    private class func hasCurrentProviderConfiguration(_ manager: NETunnelProviderManager) -> Bool {
        ...
        guard (config["schema"] as? String) == "obstaclebridge.ios.packet-tunnel.v1" else {
            return false
        }
        let derived = deriveTunnelNetworkSettings()
        guard let network = config["network_settings"] as? [String: Any] else {
            return false
        }
        let addr = network["tunnel_address"] as? String
        let prefix = (network["tunnel_prefix"] as? NSNumber)?.intValue
        let included = network["included_routes"] as? [String]
        let excluded = network["excluded_routes"] as? [String]
        let addr6 = network["tunnel_address6"] as? String
        let prefix6 = (network["tunnel_prefix6"] as? NSNumber)?.intValue
        let included6 = network["included_routes6"] as? [String]
        let excluded6 = network["excluded_routes6"] as? [String]
        let mtu = (network["mtu"] as? NSNumber)?.intValue
        return addr == derived.tunnelAddress
            && prefix == derived.tunnelPrefix
            && included == derived.includedRoutes
            && excluded == derived.excludedRoutes
            && addr6 == derived.tunnelAddress6
            && prefix6 == derived.tunnelPrefix6
            && included6 == derived.includedRoutes6
            && excluded6 == derived.excludedRoutes6
            && mtu == derived.mtu
    }
    ...
    private class func applyNetworkOverride(
        ...
        guard let override = payload["ios_tunnel_network"] as? [String: Any] else {
            return base
        }
