...
- authenticated-session failure recovery is distinct from wrong-secret retry: initial PSK mismatch stays on the bounded handshake retry path, while a post-authentication failure reports `Disconnected` and ChannelMux owns lower-stack rotation; `secure_link_recover_after_failure` and `secure_link_recover_delay_seconds` remain accepted compatibility settings and do not assign reconnect ownership to SecureLink
- the admin/API surface retains `recovery_enabled`, `recovery_delay_sec`, `recovery_reconnect_sec`, and `next_recovery_reconnect_unix_ts` as false/empty compatibility fields; ChannelMux owns failed-session rotation and its lifecycle/epoch diagnostics identify the active recovery path
