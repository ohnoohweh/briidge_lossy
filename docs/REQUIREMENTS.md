...
- `REQ-AUT-020`: Compression-enabled secure-link peers shall interoperate even when client and server use different local compression settings (for example different `compress_layer_min_bytes`, `compress_layer_level`, and allowed `compress_layer_types`). The peer-client settings shall control the client-to-server effective send profile, while the peer server shall decode those frames from the wire-level compression signal without requiring mirrored local thresholds or levels. The peer server may still use its own local settings for server-to-client traffic, as long as both peers support the same compression framing and decode guardrails.
