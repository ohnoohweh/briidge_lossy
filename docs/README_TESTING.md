...
- Apple SecureLink runtime probes compile the CryptoKit-free Core transcript source with the shared targets and source-guard the Apple codec/runtime use of its key-derivation, proof-message construction, and complete PSK frame namespace, so byte-level protocol ownership cannot silently fork again
