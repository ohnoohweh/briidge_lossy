...
| `tests/unit/test_telemetry.py`, `swift/Tests/ObstacleBridgeCoreTests/ObstacleBridgeTelemetryTests.swift` | `ARC-CMP-005` | `REQ-ADM-001` | Python and the portable Swift core share the redacted `telemetry/v1` event and batch contract. The Swift suite reads the canonical documentation vector and rejects malformed, non-allowlisted, and oversize values before later spool or transport work. | `pytest -q tests/unit/test_telemetry.py; swift test --filter ObstacleBridgeTelemetryTests` |
