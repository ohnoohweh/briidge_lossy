...
- Core Swift SecureLink coverage completes the portable PSK rekey hello/reply/commit/done exchange, holds client application sends between authenticated commit and done, verifies the new session starts both directional protected-data counters at `1`, and rejects a protected frame retained from the replaced generation
...
pytest -q tests/integration/test_overlay_e2e.py -k myudp_runtime_probe_recovers_composed_faults_rollover_and_control_idle
...
| `test_overlay_e2e_python_peer_linux_swift_myudp_runtime_probe_recovers_composed_faults_rollover_and_control_idle` | `REQ-MYU-001`, `REQ-MYU-002`, `REQ-MYU-005`, `REQ-MYU-006`, `ARC-CMP-001` | The built foreground Linux Swift client recovers when an independent Python peer composes repeated DATA loss with delayed, duplicated, reverse-ordered multi-datagram replies across the `65535 -> 1` counter rollover, then observes the resulting Core CONTROL and IDLE effects |
