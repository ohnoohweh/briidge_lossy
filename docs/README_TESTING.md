...
| `tests/unit/test_runner_events.py` | `ARC-CMP-004` | `PROC-TST-002` | Restart and shutdown events must bind to the active event loop correctly, and desktop TUN entrypoints must re-enter the same runner path with elevation plus operator-facing privilege notices when required | `pytest -q tests/unit/test_runner_events.py` |
