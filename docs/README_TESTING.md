...
- R005.4b device acceptance: the signed normal app plus `IPServer` extension is installed and launched on a physical iPhone, authenticates to a host-side peer, and is inspected through its overlay-published WebAdmin TCP service. This replaces simulator acceptance because the available simulator hosts cannot reliably supply the required resources; the simulator test remains an optional, narrower smoke.
