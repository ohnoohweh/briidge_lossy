...
- current observability includes mode, authenticated flag, `rekey_in_progress`, `failure_code`, `failure_reason`, `failure_detail`, `failure_unix_ts`, `failure_session_id`, `consecutive_failures`, `retry_backoff_sec`, `next_retry_unix_ts`, `handshake_attempts_total`, `last_event`, `last_event_unix_ts`, `last_authenticated_unix_ts`, `authenticated_sessions_total`, and `rekeys_completed_total`
...
| `tests/unit/test_admin_web_payloads.py` | `ARC-CMP-005` | `REQ-AUT-004`, `REQ-AUT-009`, `PROC-TST-002` | Admin status and peer payload builders must preserve secure-link visibility fields so WebAdmin rendering can expose encryption-layer and operational-diagnostics state consistently | `pytest -q tests/unit/test_admin_web_payloads.py` |
| `tests/unit/test_secure_link_psk.py` | `ARC-CMP-006` | `REQ-AUT-001`, `REQ-AUT-002`, `REQ-AUT-003`, `REQ-AUT-006`, `REQ-AUT-007`, `REQ-AUT-008`, `REQ-AUT-009`, `PROC-TST-002` | PSK handshake, wrong-secret rejection, bounded retry backoff, stronger operational diagnostics, malformed/out-of-order fail-closed behavior, per-peer routing, rekey rotation, and nonce/counter lifecycle guards must remain internally consistent at the secure-link layer boundary | `pytest -q tests/unit/test_secure_link_psk.py` |
