...
- current overlay failover coverage now also pins permanent ordered-peer rotation for stream transports through [tests/unit/test_stream_peer_rotation.py](../tests/unit/test_stream_peer_rotation.py) and mixed transport resolution/retry checks in [tests/unit/test_peer_resolution.py](../tests/unit/test_peer_resolution.py), keeping `myudp`, `tcp`, `ws`, and `quic` aligned on "advance to the next candidate" behavior instead of one probe cycle followed by watchdog-only waiting
...
- Admin Web and parity:
  ...
  - [ios/tests/test_macos_swift_host_runner.py](../ios/tests/test_macos_swift_host_runner.py)
- current Swift/Python parity coverage now also includes [tests/unit/test_swift_layer_readiness_parity.py](../tests/unit/test_swift_layer_readiness_parity.py), which source-guards the shared layered-readiness contract so Swift overlay owners continue to publish `connection_layers` and top-layer `app_ready`, and the Swift admin surfaces continue to consume that contract instead of falling back to raw transport-up state
...
| `test_overlay_e2e_tcp_secure_link_psk_authenticated_failure_recovers_with_reconnect` | `REQ-AUT-007`, `REQ-AUT-008`, `REQ-AUT-009` | Verify an already-authenticated client-side SecureLink failure recovers by reconnecting the lower transport | After malformed SecureLink input is injected into the authenticated TCP client, `/api/peers` reports `state=failed`, `failure_reason=decode`, and recovery scheduling/starting observability; after the configured recovery delay, reconnect survives runner disconnect reset handling, installs a new secure-link session id, and protected probe traffic succeeds again | `pytest -q tests/integration/test_overlay_e2e.py -k authenticated_failure_recovers_with_reconnect` |
...
|---|---|---|---|---|
| `tests/unit/test_channel_mux_listener_mode.py` | `ARC-CMP-003` | `REQ-MUX-003`, `REQ-MUX-004`, `REQ-MUX-005`, `REQ-MUX-009`, `PROC-TST-002` | Listener mode must ignore ambiguous local publishing, parse service specs consistently, manage remote catalog install/replace/cleanup correctly without falling back to listener-global shortcuts, keep DEBUG-only UDP path diagnostics available for service-traffic troubleshooting, route TCP target dials through shared system-proxy CONNECT egress when configured, and recover or explicitly report shared-TUN inbound ownership loss | `pytest -q tests/unit/test_channel_mux_listener_mode.py` |
...
| `tests/unit/test_secure_link_psk.py` | `ARC-CMP-006` | `REQ-AUT-001`, `REQ-AUT-002`, `REQ-AUT-003`, `REQ-AUT-006`, `REQ-AUT-007`, `REQ-AUT-008`, `REQ-AUT-009`, `REQ-AUT-010`, `PROC-TST-002` | PSK handshake, wrong-secret rejection, bounded retry backoff, post-authentication reconnect recovery through runner reset/epoch callbacks, layered transport-vs-secure-link connection state during rehandshake, stronger operational diagnostics, malformed/out-of-order fail-closed behavior, peer-confirmed authentication state, watchdog timeout for locally authenticated but unconfirmed handshakes, per-peer routing, frame/time/operator rekey rotation, and nonce/counter lifecycle guards must remain internally consistent at the secure-link layer boundary | `pytest -q tests/unit/test_secure_link_psk.py` |
...
| `tests/unit/test_build_synology_spk.py` | `ARC-CMP-004` | `REQ-RUN-002`, `PROC-TST-002` | The first-pass Synology packaging builder must emit a DSM-style `.spk` with package metadata, lifecycle scripts, the shared ObstacleBridge app icon, and a payload archive that contains the Python runtime sources, WebAdmin assets, default persistent-config seed files, and the helper-interpreter preparation script used by the current Synology privilege prototype. The same coverage now also pins the declared `python314` package dependency in SPK metadata so the maintained Synology runtime contract stays aligned with the wrapper output. | `pytest -q tests/unit/test_build_synology_spk.py` |
...
|---|---|---|---|
| `tests/unit/test_channel_mux_listener_mode.py` | ChannelMux listener semantics | Verify listener mode ignores ambiguous local config, parses service specs correctly, manages remote catalogs/lifecycle correctly, emits DEBUG-only UDP path diagnostics for service traffic troubleshooting, and recovers or reports shared-TUN inbound ownership loss with explicit drop diagnostics | `pytest -q tests/unit/test_channel_mux_listener_mode.py` |
...
| `tests/unit/test_peer_resolution.py` | Host resolution behavior | Verify localhost fallback, non-localhost resolution error behavior, and UDP `prefer-ipv6` peer-candidate/fallback handling, including native IPv4 literal resolution before IPv6 socket send mapping | `pytest -q tests/unit/test_peer_resolution.py` |
| `tests/unit/test_build_synology_spk.py` | Synology SPK wrapper build | Verify the first-pass DSM package builder emits package metadata, lifecycle scripts, the shared app icon, a declared `python314` package dependency, and a payload archive containing the Python runtime, WebAdmin assets, default config seed files, and the helper-interpreter preparation script | `pytest -q tests/unit/test_build_synology_spk.py` |
...
| `tests/unit/test_tun_helper_client_server.py` | TUN helper local socket seam | Verify helper handshake, authenticated packet echo, snapshot fetch, scaffolded apply/remove control round trips, wrong-token rejection, and disconnect reporting over the local client/server seam | `pytest -q tests/unit/test_tun_helper_client_server.py` |
| `tests/unit/test_tun_helper_server_entrypoint.py` | TUN helper module entrypoint | Verify helper CLI parsing, backend-name rejection, and the async helper serve contract behind `python -m obstacle_bridge.bridge_tun_helper_server` | `pytest -q tests/unit/test_tun_helper_server_entrypoint.py` |
