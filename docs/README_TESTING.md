...
| `tests/unit/test_ws_payload_mode.py` | `ARC-CMP-001`, `ARC-CMP-005` | `REQ-OVL-004`, `REQ-OVL-006`, `REQ-OVL-007`, `REQ-WSP-006`, `REQ-WSP-010`, `REQ-WSP-011`, `REQ-WSP-012`, `REQ-LIFE-002`, `PROC-TST-002` | WS payload encoding, client-advertised payload-mode negotiation, grouped `semi-text-shape` transfer form, encoded-frame-aware client receive sizing for text payload modes, tx timing, reconnect grace, dual-stack IPv6 wildcard listener setup, direct-path HTTP preflight body download and refusal behavior, selected-candidate handling for comma-separated peer lists, user-visible bootstrap/open failure reporting, platform-default proxy resolution, compression config, and debug static HTTP behavior must stay internally consistent | `pytest -q tests/unit/test_ws_payload_mode.py` |
