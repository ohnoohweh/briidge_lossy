...
- current observability includes mode, authenticated flag, `rekey_in_progress`, `failure_code`, `failure_reason`, `failure_detail`, and `failure_unix_ts`
...
| `test_overlay_e2e_tcp_secure_link_psk_wrong_secret_rejected` | `REQ-AUT-003`, `REQ-AUT-004` | Verify mismatched PSKs prevent the protected data phase | Client and server stay not connected, the probe does not succeed, and the admin/API reports `secure_link.state=failed` with `failure_code=1`, `failure_reason=bad_psk`, and a human-readable `failure_detail` while both processes remain alive and observable for debugging | `pytest -q tests/integration/test_overlay_e2e.py -k tcp_secure_link_psk_wrong_secret_rejected` |
...
Unit coverage currently collects `74` tests from `tests/unit/`.
...
| `tests/unit/test_secure_link_psk.py` | `ARC-CMP-006` | `REQ-AUT-001`, `REQ-AUT-002`, `REQ-AUT-003`, `REQ-AUT-006`, `REQ-AUT-007`, `PROC-TST-002` | PSK handshake, wrong-secret rejection, malformed/out-of-order fail-closed behavior, per-peer routing, rekey rotation, and nonce/counter lifecycle guards must remain internally consistent at the secure-link layer boundary | `pytest -q tests/unit/test_secure_link_psk.py` |
