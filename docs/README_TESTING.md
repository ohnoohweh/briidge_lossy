...
|---|---|---|---|---|
| `tests/unit/test_channel_mux_listener_mode.py` | `ARC-CMP-003` | `REQ-MUX-003`, `REQ-MUX-004`, `REQ-MUX-005`, `PROC-TST-002` | Listener mode must ignore ambiguous local publishing, parse service specs consistently, manage remote catalog install/replace/cleanup correctly without falling back to listener-global shortcuts, and keep DEBUG-only UDP path diagnostics available for service-traffic troubleshooting | `pytest -q tests/unit/test_channel_mux_listener_mode.py` |
...
| `tests/unit/test_embeddable_core.py` | `ARC-CMP-004` | `PROC-TST-002`, `REQ-PRX-001` | Embeddable runtime helpers must build runtime arguments from in-memory config without the CLI process entrypoint, expose the proxy-provider config schema used by WebAdmin, start configured proxy-provider listeners, expose proxy runtime status snapshots, expose a stable `ObstacleBridgeClient` lifecycle facade, and preserve packet boundaries through the `PacketIO` abstraction used by future platform TUN adapters | `pytest -q tests/unit/test_embeddable_core.py` |
...
| `tests/unit/test_proxy_server.py` | `ARC-CMP-001` | `REQ-PRX-001`, `PROC-TST-002` | The Python proxy server must mirror the Swift provider's HTTP rewrite/auth and SOCKS5 CONNECT handshake behavior while exposing listener/counter/auth snapshots for non-extension runtime use | `pytest -q tests/unit/test_proxy_server.py` |
