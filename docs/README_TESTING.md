...
- ChannelMux TCP target egress through Windows system proxy settings is pinned in [tests/unit/test_channel_mux_listener_mode.py](../tests/unit/test_channel_mux_listener_mode.py); UDP target egress remains direct until a UDP-capable proxy relay is introduced.
...
| `tests/unit/test_embeddable_core.py` | `ARC-CMP-004` | `PROC-TST-002`, `REQ-PRX-001`, `REQ-MUX-009` | Embeddable runtime helpers must build runtime arguments from in-memory config without the CLI process entrypoint, accept the nested ChannelMux egress alias, expose the proxy-provider config schema used by WebAdmin, start configured proxy-provider listeners, expose proxy runtime status snapshots, expose a stable `ObstacleBridgeClient` lifecycle facade, and preserve packet boundaries through the `PacketIO` abstraction used by future platform TUN adapters | `pytest -q tests/unit/test_embeddable_core.py` |
