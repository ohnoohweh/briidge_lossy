...
| `tests/unit/test_tun_helper_settings.py` | `ARC-CMP-004` | `PROC-TST-002`, `REQ-LIFE-010` | TUN-helper execution settings must parse defaults, preserve sectioned and CLI-shaped config values, default helper mode to the native Linux backend, and reject helper mode on unsupported platforms before runtime wiring is added | `pytest -q tests/unit/test_tun_helper_settings.py` |
...
| `tests/unit/test_tun_helper_settings.py` | TUN helper config surface | Verify helper execution defaults, section parsing, CLI-shaped config parsing, native-backend defaulting, and Linux-only helper-mode gating | `pytest -q tests/unit/test_tun_helper_settings.py` |
