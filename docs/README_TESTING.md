...
| `tests/unit/test_debug_logging_aliases.py` | `ARC-CMP-005` | `PROC-TST-002` | Logging alias configuration must reach the intended websocket-related loggers, the secure-link logger must stay quiet at `WARNING` by default unless the operator explicitly raises it, and the optional UDP logging path must reconstruct records, provide bounded receiver-owned Admin log queries, and drop transport failures without raising into the runtime | `pytest -q tests/unit/test_debug_logging_aliases.py` |
