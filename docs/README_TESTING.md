...
| `tests/unit/test_peer_resolution.py` | `ARC-CMP-004` | `REQ-OVL-007`, `PROC-TST-002` | Localhost resolution fallback must behave deterministically while non-localhost failures still propagate, and UDP `prefer-ipv6` multi-candidate handling must retain the preferred IPv6 candidate across immediate send errors while preserving IPv4 fallback through IPv4-mapped sends on IPv6 sockets | `pytest -q tests/unit/test_peer_resolution.py` |
...
| `tests/unit/test_peer_resolution.py` | Host resolution behavior | Verify localhost fallback, non-localhost resolution error behavior, and UDP `prefer-ipv6` peer-candidate/fallback handling | `pytest -q tests/unit/test_peer_resolution.py` |
