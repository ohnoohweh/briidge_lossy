...
Integration coverage currently lives in [tests/integration/test_overlay_e2e.py](../tests/integration/test_overlay_e2e.py), [tests/integration/test_linux_elevated.py](../tests/integration/test_linux_elevated.py), and [tests/integration/test_windows_elevated.py](../tests/integration/test_windows_elevated.py) and collects `142` tests.
...
| `test_overlay_e2e_tcp_secure_link_psk_compress_layer_mismatched_peer_settings` | `REQ-AUT-001`, `REQ-AUT-002`, `REQ-AUT-004`, `REQ-AUT-009`, `REQ-AUT-020`, `PROC-TST-002` | Verify secure-link TCP peers stay interoperable when client and server use different local compression settings | Client/server authenticate and pass protected probe traffic, the client-selected effective send profile compresses at least one frame, the server peer row reports `decompress_ok_total>=1`, and the server can keep its own reverse-direction compression counter at `0` under stricter local send thresholds, proving peer-server compression settings do not need to mirror the peer-client settings for the client-to-server path | `pytest -q tests/integration/test_overlay_e2e.py -k mismatched_peer_settings` |
...
Unit coverage currently collects `177` tests from `tests/unit/`.
...
| `tests/unit/test_compress_layer.py` | `ARC-CMP-007` | `PROC-TST-002` | Compression wrapper must preserve mux framing semantics, apply strict no-gain bypass on send, and fail closed for malformed or over-cap compressed payloads while keeping decode counters consistent | `pytest -q tests/unit/test_compress_layer.py` |
...
| `tests/unit/test_compress_layer.py` | Compression-layer framing behavior | Verify mux `mtype` high-bit signaling, profitable compress-or-bypass decisions, and fail-closed decompression guardrails | `pytest -q tests/unit/test_compress_layer.py` |
