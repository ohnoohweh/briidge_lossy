...
| `test_reconnect_regression` | Reconnect lifecycle regression | Verify client reconnect loops can be started again after a prior reconnect task has already completed, so repeated disconnects still trigger fresh reconnect attempts | `pytest -q tests/integration/test_reconnect_regression.py` |
...
| `test_reconnect_regression` | `REQ-LIFE-002`, `REQ-OVL-007` | Focused reconnect-task lifecycle guard: proves the legacy stale-task behavior fails after a second disconnect and the delivered fix retries again |
