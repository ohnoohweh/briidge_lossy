...
- Apple SecureLink runtime probes compile the CryptoKit-free Core transcript source with the shared targets and source-guard the Apple codec’s use of its key-derivation and proof-message construction, so byte-level PSK transcript ownership cannot silently fork again
