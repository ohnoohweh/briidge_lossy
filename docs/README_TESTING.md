...
| `tests/unit/test_launcher_entrypoint.py` | `ARC-CMP-004` | `REQ-LIFE-002`, `REQ-LIFE-005`, `REQ-LIFE-008`, `PROC-TST-002` | The default module entrypoint launcher must preserve restart-loop behavior (`75` immediate retry and `77` delayed retry), forward non-launcher CLI options to `bridge.py` consistently, print the effective WebAdmin entrypoint promptly, and keep wildcard-bind reachability hints advisory and deferred until after the supervised bridge process has started | `pytest -q tests/unit/test_launcher_entrypoint.py` |
