...
| `tests/unit/test_runtime_health.py`, `tests/unit/test_runtime_qualification_capture.py`, `swift/Tests/ObstacleBridgeCoreTests/ObstacleBridgeRuntimeHealthTests.swift`, `ios/tests/test_m3_native_sources.py` | `ARC-CMP-005` | `REQ-ADM-001` | Runtime-health records remain redacted and schema-compatible between Python and Swift Core; the bounded ring preserves only its configured tail and classifies a preceding lifetime only from its final controlled-stop marker. The Python Runner persists start, 15-second heartbeat, and controlled-stop observations adjacent to its effective configuration or at an explicit environment-selected path. The iOS provider includes that Core source, persists its App Group ring, and exposes only the retained-record count and clean/unclean classification through Admin status. The physical-qualification collector records only existing redacted Admin snapshots, requires build, traffic-source, and termination-report provenance, and rejects missing health/counter/ready-SecureLink evidence. | `pytest -q tests/unit/test_runtime_health.py tests/unit/test_runtime_qualification_capture.py ios/tests/test_m3_native_sources.py -k 'runtime_health or runtime_qualification or packet_tunnel_provider'; swift test --filter ObstacleBridgeRuntimeHealthTests` |
