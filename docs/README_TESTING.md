...
- Core Swift SecureLink coverage completes the portable PSK rekey hello/reply/commit/done exchange, retransmits the cached commit for an equivalent reply, holds client application sends between authenticated commit and done, verifies the new session starts both directional protected-data counters at `1`, and rejects a protected frame retained from the replaced generation
