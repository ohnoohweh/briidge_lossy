...
| `tests/unit/test_build_synology_spk.py` | `ARC-CMP-004` | `REQ-RUN-002`, `PROC-TST-002` | The first-pass Synology packaging builder must emit a DSM-style `.spk` with package metadata, lifecycle scripts, the shared ObstacleBridge app icon, and a payload archive that contains the Python runtime sources, WebAdmin assets, and default persistent-config seed files | `pytest -q tests/unit/test_build_synology_spk.py` |
...
| `tests/unit/test_build_synology_spk.py` | Synology SPK wrapper build | Verify the first-pass DSM package builder emits package metadata, lifecycle scripts, the shared app icon, and a payload archive containing the Python runtime, WebAdmin assets, and default config seed files | `pytest -q tests/unit/test_build_synology_spk.py` |
...
| `tests/unit/test_runner_tun_helper.py` | Runner helper-mode lifecycle | Verify helper mode launches the helper module as a subprocess, exports the helper references needed by mux startup, can elevate only the native helper subprocess on Linux, exposes helper state in `/api/status`-style snapshots, and rejects unsupported platforms early | `pytest -q tests/unit/test_runner_tun_helper.py` |
