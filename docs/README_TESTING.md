...
- 2026-04-06: WebSocket payload-mode coverage now includes a regression that proves the WS peer-client connect path uses the encoded frame limit for text payload modes. The new unit test in [tests/unit/test_ws_payload_mode.py](../tests/unit/test_ws_payload_mode.py) passes on `fix/ws-securelink-server-auth-state` and fails on the older pre-fix runtime where `_ws_frame_max_size` was missing from the client connect path.
