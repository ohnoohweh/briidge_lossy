...
| `test_overlay_e2e_ws_listener_adopts_client_payload_mode_from_upgrade_request` and `tests/unit/test_ws_payload_mode.py::{test_connect_advertises_payload_mode_in_upgrade_request,test_listener_peer_uses_advertised_payload_mode_for_rx_and_tx}` | `REQ-WSP-010` | Verify the client-advertised WS payload form is adopted automatically by the listener | A client configured for `semi-text-shape` reaches `CONNECTED` against a listener left at `binary`, the overlay probe succeeds, the upgrade request carries the payload-mode advertisement, and the listener send/receive paths use the adopted per-peer codec | `pytest -q tests/integration/test_overlay_e2e.py -k ws_listener_adopts_client_payload_mode_from_upgrade_request` and `pytest -q tests/unit/test_ws_payload_mode.py -k 'connect_advertises_payload_mode_in_upgrade_request or listener_peer_uses_advertised_payload_mode_for_rx_and_tx'` |
