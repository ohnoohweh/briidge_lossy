...
- demand-driven Python ChannelMux TCP backpressure workers and Swift per-channel Network.framework send drains, including cancellation on remote close so historical TCP churn cannot retain idle polling tasks or pending completion chains
...
- shared-TUN return-path recovery: an authenticated current lifecycle epoch reconciles a stale local ChannelMux gate from outer app-ready state, while a server-owned listener admits a reply only through an active process-shared peer binding; all new, unbound, and disconnected paths remain fail-closed. The iOS owner reannounces its retained TUN OPEN only after outbound TUN traffic remains without inbound delivery.
- ChannelMux Python/Swift parity fixtures use explicit inbound DATA and CLOSE counters for server and client channels, preserving the portable Core counter/replay contract in those delivery and close cases.
- Cross-runtime Linux Swift service tests wait independently for authenticated overlay readiness and asynchronous TCP listener publication, retrying only pre-send connection refusal within a bounded window.
...
- TUN routing, shared-TUN behavior, and hook contracts:
  ...
  - [ios/tests/test_ios_swift_udp_tun_helper.py](../ios/tests/test_ios_swift_udp_tun_helper.py)
...
- current native-helper unit coverage also pins helper-owned interface address apply/remove, helper-merged excluded-route programming, non-default included-route programming, Linux policy-table full-tunnel route setup/teardown that preserves every kernel-connected local LAN (including libvirt and VMware bridges), `resolvectl`-backed DNS apply/remove, structured rollback/error reporting for partial helper-network apply failures, and client-side snapshot refresh after asynchronous helper apply failures so helper mode can own the critical host-network mutations on the real TUN device without relying on the old inline path
- current telemetry coverage pins empty nonblocking drains, recovery after a failed background flush worker, redacted Admin worker-liveness reporting, the 15-second load schedule, and the iOS-scripted password-protected Keychain identity export, verified Documents staging, shared-Keychain import, post-import staging cleanup, and both Documents/App-Group logs-only cleanup while retaining all spool and HTTPS work outside forwarding callbacks
...
- current runtime-health coverage pins the portable Core record/ring/file contract and the Python, Linux Swift, macOS Swift, and iOS owners' redacted retained-lifetime wiring, including bounded prior records that remain queryable after restart, clean-versus-unclean classification, and the exclusion of protected traffic
...
- current onboarding invite coverage includes Python and Swift parity for carrying admin web port, compression, TUN routing, mux TCP backpressure knobs, proxy-provider settings, service catalogs, secure-link PSK material, and every client-side MyUDP/TCP/QUIC/WebSocket transport option (including WebSocket peer-address overrides and the selected client bind/port) while masking the PSK in preview output
...
- the macOS Swift elevated slice requires macOS, `swiftc`, `ifconfig`, `networksetup`, `route`, and permission to create/configure `utun` interfaces; it builds the macOS app bundle, launches `ObstacleBridgeHostRunner` from inside the app bundle so the bundled Darwin hook scripts are used, pairs it with a Python `darwin-native` helper peer, verifies real Swift-owned `utun` creation plus packet-counter movement, verifies live route/DNS hook apply/remove effects plus Swift Admin TUN config/peer/global verification parity with Linux/Python, and proves the supported functional fallback. The local operator may use `--interactive-elevation` for a Terminal password prompt, or unattended CI use its scoped passwordless `sudo` rule; the bundled executable then selects reachable XPC or its Darwin-native in-process loopback fallback and still proves real `utun` packet carriage. Packaged-XPC activation, installed-artifact lifecycle, and helper-death cases carry the `macos_xpc_qualification` marker and are excluded by default while the known BTM `fullPath is nil` blocker remains unresolved; run them only with `--run-packaged-xpc-qualification` for focused resolution evidence. The wrapper grants 300 seconds per test: this includes the separately enforced 180-second cold-build qualification plus live elevated execution, and avoids killing a valid build at the former 120-second outer pytest deadline.
- the Swift route/DNS verification endpoint performs synchronous local probe and name-resolution work. Its elevated wait loop retries an individual Admin read timeout within the bounded verification window, and reports the final Admin error only if the requested TUN state never converges.
...
- R005.4b device acceptance is qualified with the signed normal app plus `IPServer` extension on a physical iPhone. It authenticates to a host-side peer and is inspected through its overlay-published WebAdmin TCP service. This replaces simulator acceptance because the available simulator hosts cannot reliably supply the required resources; the simulator test remains an optional, narrower smoke.
- The device status build object is generated at build time and carries commit, dirty, diff, and timestamp provenance into the `IPServer` response. The container app and embedded `IPServer` extension share the same numeric `CFBundleVersion`; device build warnings about mismatched versions are failures to fix before qualifying the run.
- Optional TestFlight release housekeeping records the signed archive's identity and upload reference for the R005.4b-qualified source revision; it is not a development blocker and does not repeat the device SecureLink/WebAdmin scenario.
...
- A signed physical iPhone contains the normal ObstacleBridge iOS app and its embedded Network Extension target for R005.4b product qualification. The simulator may still contain the standalone E2E app when the lane needs test-only probes.
...
- Assertions come from external effects: host peer connection state, WebSocket/TCP traffic crossing, and redacted `/api/status` plus `/api/peers` JSON read through the authenticated overlay-published device WebAdmin service. The phone's internal `127.0.0.1:18080` listener is not host-reachable and is not used as acceptance evidence.
...
| `scripts/run_macos_swift_elevated_tests.sh` | Local macOS Swift elevated functional subset | Run privileged Swift host-runner fallback coverage from the built macOS app bundle, pairing it with a Python `darwin-native` helper peer to prove real Swift-owned `utun` creation, bundled Darwin hook apply/remove, live route/DNS effects and restoration, Swift Admin TUN config/peer/global verification parity with Linux/Python, Admin helper runtime reporting, and packet-counter movement across the overlay. Known-blocked packaged-SMAppService/XPC cases are excluded; use `--run-packaged-xpc-qualification` only for focused BTM-resolution qualification. | `./scripts/run_macos_swift_elevated_tests.sh` |
...
| `tests/unit/test_debug_logging_aliases.py` | `ARC-CMP-005` | `PROC-TST-002` | Logging alias configuration must reach the intended websocket-related loggers, the secure-link logger must stay quiet at `WARNING` by default unless the operator explicitly raises it, and the optional UDP logging path must reconstruct records, provide bounded receiver-owned Admin log queries, redacted Admin telemetry status without a runner call, and drop transport failures without raising into the runtime | `pytest -q tests/unit/test_debug_logging_aliases.py` |
| `tests/unit/test_telemetry.py`, `tests/unit/test_telemetry_ios_identity.py` | `ARC-CMP-005` | `REQ-ADM-001`, `PROC-TST-002` | Telemetry v1 accepts only the redacted allowlist, uses canonical vectors, bounds the producer queue, recovers or quarantines local spool segments without reintroducing network I/O on the runtime path, starts the Python Runner's lifecycle/load producer and single-flight uploader outside forwarding callbacks, including immediate and 15-second worker-owned load emission, requires validated ordered batches before TLS-reference ingest acknowledgement, derives the installation identity from the standard client-certificate directory, and exports a password-protected iPhone Keychain identity plus public collector CA through iOS wrappers for protected enrolment and verified temporary Documents staging. It honors explicit IPv4/IPv6 collector policy, proves installation-bound mTLS acceptance plus certificate-serial revocation, retains only server-acknowledged spool delivery, rejects replay/admission exhaustion before queue work, reports bounded redacted operator spool status, and starts the isolated collector only when the strict `telemetry_server` configuration explicitly enables it | `pytest -q tests/unit/test_telemetry.py tests/unit/test_telemetry_ios_identity.py` |
| `scripts/qualify_telemetry.py` | `ARC-CMP-005` | `PROC-TST-002` | Local pre-qualification floods the bounded producer, reports p99 emit latency and drop reasons, and fails when the configured latency threshold is exceeded | `python scripts/qualify_telemetry.py` |
...
| `tests/unit/test_runner_config_persistence.py` | `ARC-CMP-004` | `REQ-ADM-009`, `PROC-TST-002` | Runtime config updates must persist registered sections, discard the retired `misc` section, protect saved secrets, and keep `telemetry_client` operational endpoint and path values visible and editable | `pytest -q tests/unit/test_runner_config_persistence.py` |
...
| `tests/unit/test_compress_layer.py`, `ios/tests/test_ios_overlay_layer_transport_adapter.py` | `ARC-CMP-007` | `REQ-AUT-020`, `PROC-TST-002` | Compression wrappers must preserve mux framing semantics, forward SecureLink peer diagnostics, apply strict no-gain bypass on send, fail closed for malformed or over-cap compressed payloads, and activate peer-server reply compression only after a peer sends a valid compressed frame while keeping decode counters consistent. The Apple adapter compiles with the Core compression source and applies it before SecureLink on send and after SecureLink on receive. | `pytest -q tests/unit/test_compress_layer.py ios/tests/test_ios_overlay_layer_transport_adapter.py` |
...
| `tests/unit/test_embeddable_core.py` | `ARC-CMP-004` | `PROC-TST-002`, `REQ-PRX-001`, `REQ-MUX-009`, `REQ-ADM-009` | Embeddable runtime helpers must build runtime arguments from registered in-memory configuration sections without the CLI process entrypoint, ignore unsectioned values, preserve the `tun_execution` helper-topology section through the shared parser/config surface, expose the proxy-provider config schema used by WebAdmin, and expose separate `telemetry_client` producer and `telemetry_server` collector schemas, including the loopback collector endpoint and client spool defaults, start configured proxy-provider listeners, expose proxy runtime status snapshots, expose a stable `ObstacleBridgeClient` lifecycle facade, and preserve packet boundaries through the `PacketIO` abstraction used by future platform TUN adapters | `pytest -q tests/unit/test_embeddable_core.py` |
...
| `tests/unit/test_runner_config_persistence.py` | Config update persistence | Verify config updates persist registered sections, discard the retired `misc` section, protect saved secrets, and keep `telemetry_client` operational values editable | `pytest -q tests/unit/test_runner_config_persistence.py` |
