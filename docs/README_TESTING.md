...
| `tests/unit/test_build_synology_spk.py` | `ARC-CMP-004` | `REQ-RUN-002`, `PROC-TST-002` | The first-pass Synology packaging builder must emit a DSM-style `.spk` with package metadata, lifecycle scripts, the shared ObstacleBridge app icon, and a payload archive that contains the Python runtime sources, WebAdmin assets, default persistent-config seed files, and the helper-interpreter preparation script used by the current Synology privilege prototype | `pytest -q tests/unit/test_build_synology_spk.py` |
...
| `tests/unit/test_build_synology_spk.py` | Synology SPK wrapper build | Verify the first-pass DSM package builder emits package metadata, lifecycle scripts, the shared app icon, and a payload archive containing the Python runtime, WebAdmin assets, default config seed files, and the helper-interpreter preparation script | `pytest -q tests/unit/test_build_synology_spk.py` |
