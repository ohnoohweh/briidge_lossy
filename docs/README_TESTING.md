...
- Apple SecureLink runtime probes compile the CryptoKit-free Core source with the shared targets and source-guard the Apple codec/runtime use of its key-derivation, proof-message construction, complete PSK frame namespace, and delegated Core client/listener lifecycle, so byte-level protocol ownership cannot silently fork again
