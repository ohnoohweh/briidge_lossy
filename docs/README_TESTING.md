...
- current TUN / Routing Admin Web coverage also pins the non-blocking verification path so peer/global internal ICMP probe checks return `pending` or cached results while background refresh runs off the admin snapshot thread, keeping the TUN page responsive under live verification
