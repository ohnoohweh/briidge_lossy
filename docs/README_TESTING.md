...
| `tests/unit/test_telemetry.py` | `ARC-CMP-005` | `REQ-ADM-001`, `PROC-TST-002` | Telemetry v1 accepts only the redacted allowlist, uses canonical vectors, bounds the producer queue, recovers or quarantines local spool segments without reintroducing network I/O on the runtime path, requires validated ordered batches before TLS-reference ingest acknowledgement, proves installation-bound mTLS acceptance plus certificate-serial revocation, and retains only server-acknowledged spool delivery | `pytest -q tests/unit/test_telemetry.py` |
