...
- the GitHub integration gate also runs the macOS Swift elevated slice when Swift-relevant files changed, with the same passwordless `sudo` preflight and the same distinction between real-machine route/DNS/ping proof and hosted-runner diagnostic verification; packaged XPC assertions are skipped only when hosted macOS leaves the helper `not_registered` with `helper service is not enabled` after registration preflight
