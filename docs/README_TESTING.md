...
- the macOS slice requires macOS, `ifconfig`, `route`, and permission to create/configure `utun` interfaces through the Python `darwin-native` helper backend
...
Integration coverage currently lives in [tests/integration/test_overlay_e2e.py](../tests/integration/test_overlay_e2e.py), [tests/integration/test_linux_elevated.py](../tests/integration/test_linux_elevated.py), [tests/integration/test_macos_elevated.py](../tests/integration/test_macos_elevated.py), and [tests/integration/test_windows_elevated.py](../tests/integration/test_windows_elevated.py) and collects `152` tests.
...
| `test_overlay_e2e_windows_elevated_tun_over_myudp_channel_open`, `test_overlay_e2e_windows_elevated_tun_over_ws_secure_link_channel_open`, the Linux elevated TUN entrypoints in [tests/integration/test_linux_elevated.py](../tests/integration/test_linux_elevated.py), and the macOS helper-native entrypoint in [tests/integration/test_macos_elevated.py](../tests/integration/test_macos_elevated.py) | `REQ-MUX-006`, `REQ-MUX-007`, `REQ-MUX-008`, `REQ-MUX-009` | Elevated host-specific TUN coverage that defends end-to-end TUN channel establishment, peer-scoped publication, wrapped-packet handling, helper-owned host-network apply/remove, and the shared-TUN server-owned switching contract on the supported privileged backends |
...
| `tests/integration/test_macos_elevated.py` | `ARC-CMP-004` | `PROC-TST-002`, `REQ-LIFE-010`, `REQ-MUX-006`, `REQ-MUX-009` | macOS elevated integration must prove the Python helper can launch the `darwin-native` backend on the privileged path, create real `utun` interfaces, report helper runtime state through Admin Web, carry real packets through the helper-backed overlay path, invoke helper-owned Darwin apply/remove hook actions for routes and DNS, report helper death, raise a manual-cleanup warning when helper-owned route or DNS state may remain after helper loss, and tear down helper-owned interfaces/routes after shutdown or helper loss | `pytest -q tests/integration/test_macos_elevated.py -m macos_elevated --run-macos-elevated` |
...
| `tests/integration/test_macos_elevated.py` | Native Darwin helper backend, elevated path | Verify the privileged Python helper subprocess can create real `utun` devices, apply/remove Darwin hook-managed route/DNS host state, expose Admin Web helper runtime status, carry packets through helper counters, report helper death, surface the manual-cleanup warning when helper-owned route/DNS state may remain after process loss, and clean up helper-owned interfaces/routes | `pytest -q tests/integration/test_macos_elevated.py -m macos_elevated --run-macos-elevated` |
