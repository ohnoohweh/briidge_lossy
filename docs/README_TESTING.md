...
| `tests/unit/test_telemetry.py`, `swift/Tests/ObstacleBridgeCoreTests/ObstacleBridgeTelemetryTests.swift` | `ARC-CMP-005` | `REQ-ADM-001` | Python and the portable Swift core share the redacted `telemetry/v1` event and batch contract. The Swift suite reads the canonical documentation vector, verifies bounded priority-aware producer loss, and exercises checksum-spool recovery and acknowledgement isolation. | `pytest -q tests/unit/test_telemetry.py; swift test --filter ObstacleBridgeTelemetryTests` |
