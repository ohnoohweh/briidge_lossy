...
| `.5c` TCP, WebSocket, and myUDP mixed-runtime parity | In progress | The built Linux executable passes TCP/WebSocket/myUDP PSK round trips against the Python peer; TCP wrong-key rejection plus malformed-frame/replay rejection now pass against independent Python peers on all three admitted transports. The configured-session adapter performs explicit rekey through Core, and a Python peer proves live rekey plus post-rekey traffic on all three transports. TCP, WebSocket, and myUDP reconnect and reauthenticate with fresh Python-peer sessions; TCP/WebSocket reject a retired-session frame after reconnect, with the corresponding myUDP proof still open. myUDP repeated-loss, delayed, duplicated, reverse-ordered reply, CONTROL, and IDLE behavior also pass. Add counter-handling probes for every admitted transport. A valid wire-sequential myUDP counter-rollover scenario remains open; a prior discontinuous `2 -> 65535` fixture was removed because it was not a valid rollover. |
...
| `R005.5c-3` Reconnect/fresh epoch | In progress | TCP, WebSocket, and myUDP reconnect to independent Python peers with new SecureLink sessions; TCP/WebSocket reject retired-session protected data. Add the equivalent myUDP rejection proof. |
| `R005.5c-4` Counter boundaries | Open | Each admitted transport demonstrates normal counter progression, exhaustion/epoch reset behavior, and a valid wire-sequential myUDP rollover (not a discontinuous synthetic jump). |
