...
| `.5c` TCP, WebSocket, and myUDP mixed-runtime parity | In progress | The built Linux executable passes TCP/WebSocket/myUDP PSK round trips against the Python peer; TCP wrong-key rejection plus malformed-frame/replay rejection now pass against independent Python peers on all three admitted transports. The configured-session adapter performs explicit rekey through Core, and a Python peer proves live rekey plus post-rekey traffic on all three transports. TCP reconnects and reauthenticates with a fresh Python-peer session; WebSocket/myUDP reconnect and old-session rejection remain open. myUDP repeated-loss, delayed, duplicated, reverse-ordered reply, CONTROL, and IDLE behavior also pass. Add counter-handling probes for every admitted transport. A valid wire-sequential myUDP counter-rollover scenario remains open; a prior discontinuous `2 -> 65535` fixture was removed because it was not a valid rollover. |
...
| `R005.5c-3` Reconnect/fresh epoch | In progress | TCP reconnects to an independent Python peer with a new SecureLink session. Add WebSocket and myUDP, then reject stale protected data from the retired session at every adapter boundary. |
