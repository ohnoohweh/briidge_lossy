...
| `.5c` TCP, WebSocket, and myUDP mixed-runtime parity | In progress | The built Linux executable passes TCP/WebSocket/myUDP PSK round trips against the Python peer; TCP wrong-key rejection plus malformed-frame/replay rejection now pass against independent Python peers on all three admitted transports. The configured-session adapter performs explicit rekey through Core, and a Python peer proves live rekey plus post-rekey traffic on all three transports. All three transports reconnect and reauthenticate with fresh Python-peer sessions and reject a retired-session frame after reconnect. myUDP repeated-loss, delayed, duplicated, reverse-ordered reply, CONTROL, IDLE, and a valid wire-sequential `65535 -> 1` rollover pass against an independent Python peer. Add TCP/WebSocket counter-handling probes. |
...
| `R005.5c-4` Counter boundaries | In progress | Core covers SecureLink exhaustion and epoch reset; independent Python evidence confirms myUDP `65535 -> 1` rollover. Add TCP/WebSocket adapter counter-boundary probes and complete transport-level epoch-reset evidence. |
