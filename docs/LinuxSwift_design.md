...
| `R005.5d-1` Lifecycle/retry | In progress | The Swift live snapshot exposes the bounded next-retry delay and TCP proves a silent authenticated Python peer reaches deadline-driven retry and a fresh authenticated epoch. Extend fresh-epoch readiness evidence to WebSocket and myUDP. |
...
| `R005.5d-1b` | Linux Swift + Python peer | Complete | A TCP Python peer deliberately remains silent after authenticated acknowledgement. The typed 50 ms receive-idle deadline fails the old epoch, publishes the bounded retry window, and reconnects without retaining it. |
| `R005.5d-1c` | Linux Swift + Python peer | In progress | TCP proves the connected publication occurs only after a fresh authenticated epoch. Extend the same timeout-driven readiness and old-epoch rejection evidence to WebSocket and myUDP, or record a justified transport-specific capability limit. |
