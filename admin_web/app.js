...
function setOnboardingMessage(message, isOk = false) {
  ['onboardingMessage', 'setupAssistantMessage', 'tokenGeneratorMessage'].forEach((id) => {
...
function openSetupAssistantGate() {
...
async function setupAssistantNextFromToken() {
  const token = String(document.getElementById('onboardingInviteInput')?.value || '').trim();
  if (!token) {
    setOnboardingMessage('Invite token is required before moving to review.');
    return;
  }
