...
Contributor note: the latest 2026-06-04 follow-ups cover two areas. First, the Linux shared CI hardening now waits for authenticated client status plus an authenticated cert-mode peer row before the WebAdmin cert-reload test sends its first prime probe. Second, shared multi-client server-side TUN switching Phase 1 now has a delivered structured config contract in [src/obstacle_bridge/bridge_channelmux.py](src/obstacle_bridge/bridge_channelmux.py): `tun -> tun` service entries may carry `options.shared_tun_ownership`, but only as exact host-address ownership metadata (`/32` IPv4, `/128` IPv6, or bare host addresses), with duplicate claims and non-TUN misuse rejected at parse time. The broader 2026-06-03 CI follow-up keeps the Linux shared and macOS Swift-backed lanes aligned with the actual test/runtime split: mixed Swift/Python host-runner overlay cases now run only on macOS, the Linux empty-config onboarding admin test now reserves its port on the same `127.0.0.1` host it binds, and the Swift parity fixture now supplies the `tx_ns` / `echo_ns` control-packet timing fields expected by the current peer-runtime API. The prior 2026-06-01 overlay-peer follow-up also hardens dual-stack fallback on IPv4-only networks: the Python `myudp` transport now rotates immediately from an unreachable preferred IPv6 peer to the next configured candidate on hard local send errors, and the Swift packet-tunnel UDP peer bridge now applies the same immediate fallback rule for parity on macOS/iOS runtimes. Focused regression coverage now includes [tests/unit/test_peer_resolution.py](tests/unit/test_peer_resolution.py), [ios/tests/test_ios_packet_tunnel_provider_probe.py](ios/tests/test_ios_packet_tunnel_provider_probe.py), [tests/unit/test_channel_mux_listener_mode.py](tests/unit/test_channel_mux_listener_mode.py), [tests/unit/test_channel_mux_swift_parity.py](tests/unit/test_channel_mux_swift_parity.py), and the cert-reload integration slice in [tests/integration/test_overlay_e2e.py](tests/integration/test_overlay_e2e.py).
