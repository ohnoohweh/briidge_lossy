...
The 2026-05-15 runtime-refactor recovery on `ios-packet-tunnel-e2e` also revalidated the shared launcher/bootstrap path plus split transport recovery with `pytest -q tests/unit` (`219 passed, 17 subtests passed`), `pytest -q ios/tests` (`61 passed`), and `pytest -q -n 16 tests/integration/test_overlay_e2e.py` (`141 passed, 2 skipped`).
