...
The latest transport-observability follow-up extends `myudp` with a transmit-delay EWMA shown next to RTT in the runtime status/dashboard surface, backed by the first-send timestamp of each acknowledged `DATA` frame and the existing delayed/lossy overlay harness. Local validation for that slice used `pytest -q tests/unit/test_requirements_unit_gaps.py -k transmit_delay` (`1 passed, 17 deselected`) and `pytest -q tests/integration/test_overlay_e2e.py -k myudp_delay_loss` (`13 passed, 131 deselected`).
