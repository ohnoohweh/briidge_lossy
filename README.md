...
- Apple SecureLink delegates its PSK frame namespace, derivation, proofs, protected-frame AEAD, and client-side handshake/data/rekey lifecycle to the pinned Core `Crypto` surface; generated projects add the product as both a target dependency and framework reference, remove stale references to deleted Core-migrated compatibility facades, while listener lifecycle migration remains in progress.
