...
This baseline also includes explicit traceability for the layered reconnect contract where the lower overlay transport can remain connected while SecureLink is still re-handshaking, Linux Swift foreground `myudp` recovery probes that retain the independent peer endpoint until its protected reply is drained and the client has emitted a Core CONTROL or IDLE acknowledgement, keep their repeated-loss peer alive until the test-owned subprocess completes, survive two independently dropped protected application copies and a delayed, duplicated, reverse-ordered multi-chunk reply while returning Core CONTROL and IDLE effects in the same protected exchange, and a macOS mixed Swift/Python `myudp` matrix that recovers a full CONTROL missing list by splitting retransmissions into wire-valid DATA batches. A valid wire-sequential end-to-end counter-rollover probe remains an explicit qualification gap.
