...
Contributor note: the latest 2026-06-05 follow-ups cover eleven areas. First, the Linux shared CI hardening now treats the first authenticated WebAdmin cert-reload prime probe as a bounded readiness phase: the shared overlay harness refreshes self-restarting client/server processes, re-establishes the authenticated admin session, re-validates the client-side authenticated cert row, and retries the first prime probe within a fixed window before the browser-path reload assertions proceed. Second, the earlier Linux shared cert-reload hardening still waits for authenticated client status, an authenticated cert-mode peer row, the forwarded TCP probe listener itself before the WebAdmin cert-reload test sends its first prime probe, and the client admin/status surface to recover before the full-material reload test asserts post-reload authenticated peer state. Third, shared multi-client server-side TUN switching now has delivered Phase 1 through Phase 5 plus a proven elevated Linux host slice in [src/obstacle_bridge/bridge_channelmux.py](src/obstacle_bridge/bridge_channelmux.py) and [tests/integration/test_linux_elevated.py](tests/integration/test_linux_elevated.py): `tun -> tun` service entries may carry `options.shared_tun_ownership` as exact host-address ownership metadata (`/32` IPv4, `/128` IPv6`, or bare host addresses), ChannelMux now materializes that metadata into per-service ownership state plus active peer/channel bindings for TUN snapshots, shared-TUN attaches can bind only to prestarted server-owned TUN services, inbound shared-TUN packets are parsed and source-checked when that ownership policy is active, local/server TUN packets are now routed only to the designated active peer channel or channels selected by destination ownership, inbound shared-TUN unicast packets can now relay directly from one active peer to another through the server without forcing a local-TUN write first, listener mode now retains and prestarts explicit `server_shared` `tun -> tun` local services even while the overlay is passively waiting for peers, and the privileged Linux elevated suite now passes with one server-owned shared TUN, two real client TUN peers, spoof rejection, and disconnect/rebind cleanup under actual `/dev/net/tun` packet I/O. Fourth, the TUN routing contract is now centralized more strictly around `TUN_routing`: Python and Swift now derive hook defaults such as `MTU`, `ENABLE_TCPMSS`, optional TUN tcpdump settings, DNS, included-route lists, and peer/subnet metadata from that one section, explicit empty server-side `tunnel_gateway` / `tunnel_gateway6` values are preserved instead of silently reverting to defaults, Linux server-side TUN hooks no longer require `PEER_ADDR`, structured `tun -> tun` service entries may omit per-service `mtu` so they inherit `TUN_routing.mtu` unless a service-level override is supplied, and the effective local excluded-route set now auto-appends the resolved active overlay peer IPv4 and IPv6 host routes so full-tunnel clients keep their own overlay transport peer off-tunnel by default across Python and Swift. Fifth, the Linux client TUN hook now waits for device creation, tolerates teardown after the interface is already gone, and installs point-to-point default routes with `onlink`, which keeps shared-TUN client bring-up alive under reconnect churn instead of failing with `Nexthop has invalid gateway`; the shipped macOS config already matches the intended one-shot lifecycle by running TUN `up` only on `on_created` and `down` on `on_stopped`, while the iOS packet-tunnel path has no equivalent shell-hook replay surface. Sixth, the Python `TUN / Routing` admin payload now exposes those runtime-effective included/excluded routes too, so the browser tab shows the live auto-appended WS peer `/32` instead of only the static config snapshot. Seventh, packet-switch component parity is now split from secure-link parity: the new [tests/unit/test_channel_mux_component_parity.py](tests/unit/test_channel_mux_component_parity.py) and [tests/fixtures/channelmux_component_runner.swift](tests/fixtures/channelmux_component_runner.swift) compile only the ChannelMux codec/TUN runtime surface, so Linux can prove Swift/Python parity for shared ownership metadata, anti-spoofing behavior, destination-based TUN routing, peer-relay decisions, core TUN component behavior, and broadcast-versus-unicast throttle-scope isolation without pulling in `CryptoKit`. Eighth, shared-TUN throttling now scopes `REQ-MUX-011` budgets to the selected peer/channel set rather than one process-wide TUN inflow bucket, and the same [tests/unit/test_channel_mux_component_parity.py](tests/unit/test_channel_mux_component_parity.py) runner now compares multi-scope Swift/Python throttle decisions so one backed-up peer path does not incorrectly consume another peer's budget. Ninth, shared-TUN robustness now keeps bounded per-service drop counters plus a bounded recent-drop buffer for spoof, routing, throttling, and oversize failures, and the TUN/routing summary now aggregates `shared_drop_total` so operators can see repeated shared-TUN failures without relying only on scrolling logs. Tenth, Admin Web now has the same component-level parity treatment: [ios/native/ObstacleBridgeShared/ObstacleBridgeAdminAPI.swift](ios/native/ObstacleBridgeShared/ObstacleBridgeAdminAPI.swift) and [ios/native/ObstacleBridgeShared/ObstacleBridgeWebAdminServer.swift](ios/native/ObstacleBridgeShared/ObstacleBridgeWebAdminServer.swift) now advertise the dedicated `tun_routing` route/topic, and the new [tests/unit/test_admin_web_component_parity.py](tests/unit/test_admin_web_component_parity.py) plus [tests/fixtures/admin_web_component_runner.swift](tests/fixtures/admin_web_component_runner.swift) let Linux compare Swift and Python Admin UI, security-advisor, TUN/routing, and admin API component behavior without relying only on the macOS host-runner lane. Eleventh, the shared Python integration harness now includes a focused two-client `myudp` listener restart regression in [tests/integration/test_overlay_e2e.py](tests/integration/test_overlay_e2e.py) that keeps one remote TCP service live, bounces the listener server on the same overlay port, and then requires fresh TCP connects to succeed again on the same reoffered ports after both peers recover.
