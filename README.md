...
Contributor note: the latest 2026-06-04 follow-ups cover three areas. First, the Linux shared CI hardening now waits for authenticated client status plus an authenticated cert-mode peer row before the WebAdmin cert-reload test sends its first prime probe. Second, shared multi-client server-side TUN switching now has both a delivered Phase 1 config contract and a delivered Phase 2 runtime bookkeeping slice in [src/obstacle_bridge/bridge_channelmux.py](src/obstacle_bridge/bridge_channelmux.py): `tun -> tun` service entries may carry `options.shared_tun_ownership` as exact host-address ownership metadata (`/32` IPv4, `/128` IPv6`, or bare host addresses), ChannelMux now materializes that metadata into per-service ownership state plus active peer/channel bindings for TUN snapshots, and disconnect/teardown cleanup now clears the runtime bindings without changing baseline 1:1 forwarding semantics. Third, packet-switch component parity is now split from secure-link parity: the new [tests/unit/test_channel_mux_component_parity.py](tests/unit/test_channel_mux_component_parity.py) and [tests/fixtures/channelmux_component_runner.swift](tests/fixtures/channelmux_component_runner.swift) compile only the ChannelMux codec/TUN runtime surface, so Linux can prove Swift/Python parity for shared ownership metadata and core TUN component behavior without pulling in `CryptoKit`. The broader 2026-06-03 CI follow-up still keeps the Linux shared and macOS Swift-backed lanes aligned with the actual test/runtime split, and the prior 2026-06-01 overlay-peer follow-up still hardens dual-stack fallback on IPv4-only networks.
