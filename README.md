...
The secure-link topic now has reserved future requirement IDs in `docs/REQUIREMENTS.md`, while the certificate/key-material input profile is documented in `docs/SYSTEM_BOUNDARY.md`. A narrow TCP+PSK prototype now exists for development/testing of the layer boundary, including a two-peer TCP listener slice, but the numeric coverage snapshot above still reflects only the currently defended delivered `REQ-*` requirements.
