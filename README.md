...
Testing statistics (see [docs/README_TESTING.md](docs/README_TESTING.md)): `155` integration tests, `202` unit tests, and `49` iOS-focused tests in `ios/tests/`. Latest focused validation for the iOS packet-tunnel background slice used `.venv/bin/python -m pytest -q ios/tests/test_ios_e2e_app_runner.py ios/tests/test_m3_native_sources.py ios/tests/test_m3_tunnel.py tests/integration/test_ios_e2e.py` and completed with `31 passed`, plus `OBSTACLEBRIDGE_RUN_IOS_SIMULATOR=1 .venv/bin/python -m pytest -q tests/integration/test_ios_simulator_e2e.py::test_ios_simulator_webadmin_and_channelmux_survive_app_backgrounding tests/integration/test_ios_simulator_e2e.py::test_ios_simulator_e2e_app_ws_secure_link_client_authenticates_and_is_visible_via_host_webadmin_api -s` and completed with `2 passed`. The iOS app/runtime slice now adds focused coverage for extension-hosted WebAdmin, ChannelMux, compression, SecureLink, simulator SecureLink authentication, config persistence through WebAdmin APIs, and WebAdmin/ChannelMux operation after the iOS app is pushed to background. Earlier focused validation includes the iOS Documents-backed WebAdmin/config/log slice, native iOS crypto bridging, compression/admin/lifecycle, SecureLink PSK, config persistence, peer-traffic/concurrent-listener, TUN WebAdmin/hook, WebAdmin service-editor/service-name, WebAdmin PSK reveal, and launcher dependency-assistance slices documented in [docs/README_TESTING.md](docs/README_TESTING.md).
