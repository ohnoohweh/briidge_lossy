...
- Apple SecureLink delegates its PSK frame namespace, derivation, proofs, and protected-frame AEAD to the pinned Core `Crypto` surface; generated projects add the product as both a target dependency and framework reference, remove stale references to deleted Core-migrated compatibility facades, while its lifecycle runtime remains explicitly separate during the shared-state migration.
