...
- The macOS ChannelMux parity runner compiles the canonical Core source with its pinned `Crypto` module, and its myUDP Python reference peer remains bound until test teardown if the post-reply acknowledgement has not arrived, preventing a loaded CI host from converting reply drain into a port-close failure.
